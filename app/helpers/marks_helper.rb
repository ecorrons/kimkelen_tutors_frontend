module MarksHelper
	...
	def format_examinations(examination, examination_instance)
		if examination_instance == "reg" && examination.first["examination_number"] == 1
			"#{ examination.first['is_absent'] == 0 ? format_mark(examination.first['mark']) : 'A'}"+" (#{ examination.first['date'].to_date unless examination.first['date'].blank? })"
		else
	...
	def format_repproveds(repproves)
		"#{ repproves.first['is_absent'] == 0 ? format_mark(repproves.first['mark']) : 'A'}"+" (#{ repproves.first['date'].to_date unless repproves.first['date'] == nil })"
	end
